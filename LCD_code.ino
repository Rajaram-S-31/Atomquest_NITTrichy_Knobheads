...
uint16_t  bg_color =0;
int powerLevel = 0;  // Variable to hold the power level input
...
void loop() {
  if (Serial.available() > 0) {
    // Read the power level from the serial input
    powerLevel = Serial.parseInt();

    // If a valid number is received (valid range from 0 to 240)
    if (powerLevel >= 0 && powerLevel <= 240) {
      // Plot needle at the received power level
      plotNeedle(powerLevel, 30);
      // Print the power level to serial monitor
      Serial.print("Power Level Set To: ");
      Serial.println(powerLevel);
    } else {
      Serial.println("Invalid Power Level. Please input a value between 0 and 240.");
    }
  }
}
